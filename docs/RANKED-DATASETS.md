...
`GET /api/v1/pvp-rankings/:league/:speciesId/teammates` ouvre la fiche de détail canonique PvPoke correspondant au format, extrait les cinq liens `.partner-pokemon .list a`, résout leurs alias par Identity Manager et persiste le résultat dans `pvp_teammate_cache`. Le navigateur serverless ne charge que PvPoke et bloque les publicités, images, médias et polices sans interrompre les scripts ni les datasets nécessaires au Team Ranker. Le calcul dispose de 48 secondes dans la Function de 60 secondes ; un dépassement reste une erreur explicite `PVP_TEAMMATE_SOURCE_TIMEOUT` et n'est pas transformé en faux résultat vide. La clé inclut le `sourceHash` du classement et le cache expire après 24 heures. Une panne d'écriture de ce cache ou des diagnostics n'annule pas un calcul source valide : la réponse signale `miss-unpersisted` et `persistenceWarnings`. Une identité non résolue reste visible comme diagnostic et n'obtient aucun asset arbitraire.
