...
test("le reader sépare le core et les quatre familles sans collision", () => {
  assert.equal(data.pokemonAssets.length, 1617);
  assert.equal(data.pokemonAssetFamilies.length, 3030);
  assert.equal(new Set(data.pokemonAssets.map((document) => document.formId)).size, 1617);
  assert.equal(
  ...
  assert.deepEqual(
    ...
    { home: 1092, shuffle: 1512, variants: 211, "location-cards": 215 },
  );
